    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl w-[95vw] max-h-[90vh] p-0 overflow-hidden bg-background border-border">
        ...
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div>
            <h2 className="text-xl font-display font-bold capitalize">{layout.type} Layout</h2>
            <p className="text-sm text-muted-foreground">{layout.description || layout.preview}</p>
          </div>
        ...
        <div className="p-6 overflow-y-auto">
          ...
          {isBeforeAfter && photos.length >= 2 && (
            <div className="max-w-2xl mx-auto">
              <BeforeAfterSlider
                beforeImage={photos[layout.beforePhotoIndex ?? 0]?.url}
                afterImage={photos[layout.afterPhotoIndex ?? photos.length - 1]?.url}
              />
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-center gap-4">
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => setSlideshowIndex(prev => Math.min(layoutPhotos.length - 1, prev + 1))}
                >
          ...
          {isGrid && !isBeforeAfter && !isCarousel && !isSlideshow && (
            <div className="space-y-4">
              ...
              <p className="text-center text-sm text-muted-foreground">
                {layoutPhotos.length}-photo grid layout
              </p>
            </div>
          )}
